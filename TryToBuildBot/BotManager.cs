    ...
    class BotManager
    {
        ...
        public async void ProcessMessage(Message message)
        {
            ...
            if (chatDictionary.ContainsKey(message.Chat.Id))
            {
                ...
                handler.ProccessMessage(message);
            ...
            else
            {
                ...
                await PosmBot.Bot.SendTextMessageAsync(message.Chat.Id, BotAnswers.InfoMessage(), false, false, 0);
                await PosmBot.Bot.SendTextMessageAsync(message.Chat.Id, "Введите логин", false, false, 0);
